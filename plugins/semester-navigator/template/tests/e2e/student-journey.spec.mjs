...
test.describe('high school support',()=>{
 ...
 test('mobile dialog stays usable and rubric support carries the right student and source criteria',async({page,workspace})=>{
   ...
   const prompt=await page.getByLabel('Request for Semester Navigator').inputValue();expect(prompt).toContain('High School Fixture (high-school, Fall 2026)');expect(prompt).toContain('Show the reasoning for each answer.');expect(prompt).toContain('Ask me to attach my draft');
