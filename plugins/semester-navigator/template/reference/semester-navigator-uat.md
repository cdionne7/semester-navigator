...
| Current application/model/runtime suites | `npm test` builds both runtimes, regenerates the release, and runs model, real HTTP, hosted-route, updater, bootstrap, and ZIP tests. Final local run: 64 passed, one native-Windows test skipped on macOS. TypeScript and ESLint passed; the dependency audit reported zero vulnerabilities. |
...
| Native Windows clean-machine setup | Required on Windows without Git, GitHub CLI, system Node, or inherited PATH. macOS tests and PowerShell text checks do not establish this result. |
...
| College forward walkthrough | Tool-executed from a copied plugin bundle, with an injected source instruction, exact/date-only/unknown deadlines, grade weights, completion/notes, source correction, and reopening. Passed; effort-display defects were found, fixed, and covered by browser regression. See [evidence](../reviews/college-forward-loop.md). |
