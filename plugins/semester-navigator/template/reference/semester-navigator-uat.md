...
| Current application/model/runtime suites | `npm test` builds both runtimes, regenerates the release, and runs model, real HTTP, hosted-route, updater, bootstrap, and ZIP tests. Final local run: 62 passed, one native-Windows test skipped on macOS. TypeScript and ESLint passed; the dependency audit reported zero vulnerabilities. |
