...
export function validateDedicatedBrowser(value) {
  const name = requireText(value, "browserProfile", 120);
  if (/^(default|profile\s*\d+|guest|incognito|inprivate|pending|unknown)$/i.test(name)) {
    fail("browserProfile must be a dedicated named browser profile, not a default profile or private session.");
  }
  return name;
}
...
export async function inspectStudentSite(options) {
  ...
  const hostingExists = await exists(hostingPath);
  const hostingSourcePath = hostingExists ? hostingPath : join(studentRoot, ".openai", "hosting.example.json");
  for (const parts of [[".semester-navigator", "profile.json"], [".semester-navigator", "site.json"], [".openai", hostingExists ? "hosting.json" : "hosting.example.json"], ["app", "student-seed.json"], ["chatgpt.md"]]) {
    let current = studentRoot;
    for (const part of ["", ...parts]) {
      current = join(current, part);
      if ((await lstat(current)).isSymbolicLink()) fail(`Student identity path contains a symbolic link: ${parts.join("/")}`);
    }
  }
  const [profile, site, hosting] = await Promise.all([
    readJson(profilePath, "profile.json"),
    readJson(sitePath, "site.json"),
    readJson(hostingSourcePath, hostingExists ? "hosting.json" : "hosting.example.json"),
  ]);

  if (!hostingExists && (site.project_id || site.status !== "not_created" || hosting.project_id)) {
    fail("A recorded Site requires its actual hosting.json manifest. A hosting example cannot replace a deployed Site binding.");
  }

  if (profile.profile_id !== site.profile_id) fail("profile.json and site.json profile IDs do not match.");
  if (options.profileId && options.profileId !== profile.profile_id) fail("The requested profile ID does not match this student workspace.");
  if (profile.display_name !== site.student_display_name || profile.instance_key !== site.instance_key) {
    fail("Student identity does not match between profile.json and site.json.");
  }
  requireText(profile.approved_local_root, "profile approved_local_root", 1024);
  requireText(site.source_root, "site source_root", 1024);
  ...
  if (profile.browser_profile !== site.browser_profile || profile.browser_profile !== profile.machine?.browser_profile) {
    fail("profile.json and site.json browser profiles do not match.");
  }
  if (profile.machine?.browser_integration) validateDedicatedBrowser(profile.browser_profile);
  if ((profile.site_access?.intended_viewer_email ?? null) !== (site.audience?.viewer_email ?? null) ||
      profile.site_access?.viewer_email_storage_approved !== site.audience?.viewer_email_storage_approved) {
    fail("The intended viewer account or storage approval does not match this student workspace.");
  }
  if (!new Set(["owner-only", "selected-users"]).has(site.access_mode) || site.access_mode !== site.audience?.mode) {
    fail("The private Site access mode is invalid or inconsistent.");
  }
  ...
  const seed = await readJson(join(studentRoot, "app", "student-seed.json"), "student seed");
  for (const [key, expected] of [["profileId", profile.profile_id], ["name", profile.display_name], ["school", profile.school]]) {
    if (seed[key] !== expected) fail(`The student seed ${key} does not match profile.json.`);
  }
  try {
    const plan = await readJson(join(studentRoot, ".semester-navigator", "plan.json"), "local student plan");
    const record = plan.plan ?? plan;
    if (record.profileId !== profile.profile_id) fail("The local plan profileId does not match profile.json.");
  } catch (error) {
    if (!error.message.includes("ENOENT")) throw error;
  }

  return { studentRoot, profilePath, sitePath, hostingPath, hostingSourcePath, hostingExists, profile, site, hosting, seed };
}
...
export async function recordStudentSite(options) {
  ...
  if (!state.hostingExists || state.hosting.project_id !== projectId) {
    fail("Sites must first write the same project ID to .openai/hosting.json.");
  }
  const url = validateUrl(options.url);
  if (!state.site.project_id && options.accessMode !== "owner-only") {
    fail("The first student Site deployment must remain owner-only.");
  }
  if (state.site.project_id && (state.site.project_id !== projectId || state.site.url !== url || state.site.access_mode !== options.accessMode)) {
    fail("A deployment update must preserve the recorded project, URL, and private audience. Record an approved access change separately.");
  }
  ...
  const updatedSite = {
    ...
    url,
    access_mode: state.site.project_id ? state.site.access_mode : "owner-only",
    audience: {
      ...(state.site.audience ?? {}),
      mode: state.site.project_id ? state.site.access_mode : "owner-only",
      allowed_viewer_emails: state.site.audience?.allowed_viewer_emails ?? [],
...
export async function recordStudentSiteAccess(options) {
  ...
  const browserProfile = validateDedicatedBrowser(options.browserProfile);
