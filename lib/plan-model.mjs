...
export function normalizePlan(raw, expectedProfileId) {
  ...
  const sources = unique(
    list(r.sources, "Sources", 100).map((value) => {
      const s = object(value, "Source");
      const result = {
        id: id(s.id, "Source ID"),
        title: text(s.title, "Source title"),
        url: webUrl(s.url, "Source link"),
        type: text(s.type, "Source type", 80, "manual"),
        status: choice(
          s.status,
          "Source status",
          ["not-connected", "manual", "connected", "needs-sign-in"],
          "not-connected",
        ),
        lastChecked: timestamp(s.lastChecked, "Source last checked"),
        verified: boolean(s.verified, "Source verification"),
      ...
      if (
        ...
        (!result.verified || !result.lastChecked)
      )
        fail(
          `Connected source ${result.title || result.id} requires a verified account check and last-checked timestamp.`,
        );
      return result;
    }),
    "Sources",
...
export function mergeImportedPlan(currentRaw, incomingRaw, baselineRaw) {
  ...
  const incoming = normalizePlan(incomingRaw, current.profileId);
  ...
  const mergeObject = (local, remote, base, kind) => {
    ...
    for (const [key, value] of Object.entries(remote)) {
      ...
      if (
        !baseline &&
        ["course", "task", "resource", "component"].includes(kind)
      ) {
